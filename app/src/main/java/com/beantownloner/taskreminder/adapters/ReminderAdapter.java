...
public class ReminderAdapter extends RecyclerView.Adapter<ReminderAdapter.ViewHolder> {
    ...
    public ReminderAdapter(Context context, List<Notifications> reminderList) {
        this.context = context;
    ...
    public void onBindViewHolder(final ViewHolder viewHolder, int position) {
        ...
        viewHolder.title.setText(reminderList.get(position).getNotificationTitle());
        viewHolder.content.setText(reminderList.get(position).getNotificationContent());
        viewHolder.time.setText(DateAndTimeUtil.toStringReadableTime(calendar, context));
        int iconResId = context.getResources().getIdentifier(reminderList.get(position).getNotificationIcon(), "drawable", context.getPackageName());
        viewHolder.icon.setImageResource(iconResId);
        GradientDrawable bgShape = (GradientDrawable) viewHolder.circle.getDrawable();
        bgShape.setColor(Color.parseColor(reminderList.get(position).getNotificationColor()));

        viewHolder.view.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                Intent intent = new Intent(context, ViewActivity.class);
                intent.putExtra("NOTIFICATION_ID", reminderList.get(viewHolder.getAdapterPosition()).getNotificationID());
    ...
    public List<Notifications> getReminderList() {
        return reminderList;
    }

    public void setReminderList(List<Notifications> reminderList) {
        this.reminderList = reminderList;
    }
}